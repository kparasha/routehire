...
const bin = join(dirname(fileURLToPath(import.meta.url)), "../bin/routehire-mcp.mjs");
const child = spawn(process.execPath, [bin], {
  stdio: ["pipe", "pipe", "pipe"],
  env: { ...process.env, ROUTEHIRE_API_URL: "http://127.0.0.1:3000" },
});
...
const timer = setTimeout(() => {
  ...
  console.log("routehire-mcp bin smoke: ok");
  process.exit(0);
}, 1500);
